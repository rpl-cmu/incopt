...
#include <cpp/thirdparty/gpmp2/geometry/DynamicVector.h>
#include <cpp/thirdparty/gpmp2/geometry/DynamicLieTraits.h>
#include <cpp/thirdparty/gpmp2/geometry/ProductDynamicLieGroup.h>
...
namespace gtsam {
template<> struct traits<incopt::Pose2Vector> : internal::DynamicLieGroupTraits<incopt::Pose2Vector> {

  static void Print(const incopt::Pose2Vector& m, const std::string& s = "") {
    std::cout << s;
    m.first.print("Pose: \n");
    m.second.print("Configuration: \n");
  }

  static bool Equals(const incopt::Pose2Vector& m1, const incopt::Pose2Vector& m2, double tol = 1e-8) {
    return traits<Pose2>::Equals(m1.first, m2.first, tol) &&
        traits<incopt::DynamicVector>::Equals(m1.second, m2.second, tol);
  }
};

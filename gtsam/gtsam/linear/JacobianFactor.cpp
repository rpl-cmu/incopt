...
void JacobianFactor::JacobianFactorHelper(const GaussianFactorGraph& graph,
  ...
  //for(auto v: varDims ) std::cout << v << std::endl;
...
std::pair<GaussianConditional::shared_ptr, JacobianFactor::shared_ptr> EliminateQR(
  ...
  gttic(EliminateQR);
  //std::cout << ":::::::::Inside EliminateQR::::::::::::" << std::endl;
  ...
  //std::cout << "Ab.cols() " << Ab.cols() << " Ab.row() " << Ab.rows() << std::endl;
  if (jointFactor->model_) {
    //std::cout << "Performing  noise model QR " << std::endl;
    ...
    //std::cout << "Performing inplace_QR " << std::endl;

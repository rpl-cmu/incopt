...
struct GTSAM_EXPORT DeltaImpl {
  ...
  static size_t UpdateGaussNewtonDeltaOtherRHS(const ISAM2::Roots& roots, 
                                               const KeySet& replacedKeys,
                                               double wildfireThreshold, 
                                               VectorValues* delta,
...
struct GTSAM_EXPORT UpdateImpl {
  ...
  void pushBackFactors(const NonlinearFactorGraph& newFactors,
    ...
    *newFactorsIndices = nonlinearFactors->add_factors(
    ...
    //std::cout << "Inside pushBackFactors " << std::endl;
    //for(auto fi: *newFactorsIndices) std::cout << fi << std::endl;
  ...
  KeySet gatherRelinearizeKeys(const ISAM2::Roots& roots,
                               ...
                               KeySet* markedKeys,
                               boost::optional<bool> enableConstraints_ = boost::none) const {

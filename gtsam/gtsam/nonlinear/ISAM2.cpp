...
void ISAM2::recalculate(const ISAM2UpdateParams& updateParams,
  ...
  if (!result->markedKeys.empty() || !result->observedKeys.empty()) {
    ...
    if(debug_print_)  std::cout << "affectedKeys.size()  " << affectedKeys.size()  << " | " << "theta_.size() " << theta_.size() << std::endl;
...
void ISAM2::recalculateBatch(const ISAM2UpdateParams& updateParams,
  ...
  gttic(add_keys);
  if(debug_print_)  cout << "Inside recalculateBatch " << endl;
  //cout << "^^^^^^^^^^^^^^^Inside recalculateBatch^^^^^^^^^^^^^^^^^ " << endl;
  ...
  auto etree =  GaussianEliminationTree(*linearized, affectedFactorsVarIndex, order);

  auto JT =  ISAM2JunctionTree(etree);
  if (enableConstraints_) JT.setEnableConstraints(true);
  ISAM2BayesTree::shared_ptr bayesTree = JT.eliminate(params_.getEliminationFunction()).first;
  gttoc(eliminate);
...
void ISAM2::recalculateIncremental(const ISAM2UpdateParams& updateParams,
  ...
  if(debug_print_) cout << "^^^^^^^^^^^^^^^Inside recalculateIncremental^^^^^^^^^^^^^^^^^ " << endl;
  ...
  // Keep track of the original non linear factors indices that were relinarized 
  FastVector<int> factorIndices;
  GaussianFactorGraph factors =
      relinearizeAffectedFactors(updateParams, affectedAndNewKeys, relinKeys, &factorIndices);
  ...
  if (debug) ordering.print("\n[iSAM2::recalculate::inc] ordering: \n");

  factorIndices.insert(factorIndices.end(), factors.size() - factorIndices.size(), -1);
  GaussianEliminationTree etree(factors, affectedFactorsVarIndex, ordering, factorIndices);
...
double ISAM2::avgErrorBConstrained() {
  ISAM2Constrained isam2cst(params_, debug_print_, constraintPenaltyParams_);
  Values currValues = calculateEstimateConstrained();
  double nonLinearError = isam2cst.avgErrorBConstrained(nonlinearFactors_, currValues, numRowsHardConstraints_);
  return nonLinearError;
}

double ISAM2::sumErrorConstrained() {
  ISAM2Constrained isam2cst(params_, debug_print_, constraintPenaltyParams_);
  Values currValues = calculateEstimateConstrained();
  double nonLinearError = isam2cst.sumErrorConstrained(nonlinearFactors_, currValues);
  return nonLinearError;
}

double ISAM2::avgErrorConstrained() {
  ISAM2Constrained isam2cst(params_, debug_print_, constraintPenaltyParams_);
  Values currValues = calculateEstimateConstrained();
  double nonLinearError = isam2cst.avgErrorConstrained(nonlinearFactors_, currValues);
  return nonLinearError;
}

void ISAM2::updateDeltaConstrained(double* res_nonlinConstraintErrorAfter, bool forceFullSolve) {
  gttic(solve_constraints_full);
  if (enableConstraints_) {
    ISAM2Constrained isam2cst(params_, debug_print_, constraintPenaltyParams_);
    int iter = 0;
    if (numNonLinearFactorsOld_ != nonlinearFactors_.size()) {
      for (size_t i = numNonLinearFactorsOld_ ; i < nonlinearFactors_.size(); i++) {
        if(nonlinearFactors_.at(i)->isConstraintFactor()) {
          numRowsHardConstraints_ += nonlinearFactors_.at(i)->dim();
        }
      }
      numNonLinearFactorsOld_ = nonlinearFactors_.size();
    }

    Cliques stopNodes = orphans_;
    while(iter < this->numInnerIter_) {
      std::queue<sharedClique> upwardPassQueue;
      if(debug_print_) std::cout << "Upward Pass start.." << std::endl;
      gttic(upperPass);
      isam2cst.upwardPass(roots_[0], nonlinearFactors_, theta_, stopNodes);
      gttoc(upperPass);
      if(debug_print_) std::cout << "Upward Pass end .." << std::endl;
      if(debug_print_) std::cout << "Downward Pass start.." << std::endl;
      gttic(downwardPass);
      std::tuple<bool, FastList<ISAM2::sharedClique>> ret = isam2cst.downwardPass(&delta_, roots_, deltaReplacedMask_, nonlinearFactors_, 
                                                      this->constraintDeltaMaxStep_, iter, numRowsHardConstraints_,
                                                      forceFullSolve, capByDim_);
      gttoc(downwardPass);
      bool breakInner = get<0>(ret);
      stopNodes = get<1>(ret);
      if(debug_print_) std::cout << "Downward Pass end.." << std::endl;
      if(breakInner) { 
        if(debug_print_) std::cout << "Breaking at iteration " << iter << std::endl;
        break;
      }
      iter++;
    }
    if(debug_print_) delta_.print();
  }
  gttoc(solve_constraints_full);
  tictoc_print();
}
...
ISAM2Result ISAM2::update(const NonlinearFactorGraph& newFactors,
  ...
  // If new factors are added, initialize the dualV vectors to zero
  if(enableConstraints_){
      for (auto f: newFactors) {
        int dim = f->dim();
        FastVector<double> dualV(dim, 0.0);
        f->setdualV(dualV);
        FastVector<double> rho(dim, 2.0);
        f->setRho(rho);
      }
  }
  // Update delta if we need it to check relinearization later0
  if (update.relinarizationNeeded(update_count_) || params_.forceConstrainedDeltaUpdate) {
    if(debug_print_) std::cout << "Updating delta.. " << std::endl;
    if(enableConstraints_) {
      // Initialize the dualV for new factors 
      if(roots_.size() > 1) 
        throw std::runtime_error("Cannot handle multiple roots");
      else if(roots_.size() == 1) 
        updateDeltaConstrained(&result.nonLinearConstrainedError, updateParams.forceFullSolve);
    }
    else {
  ...
  if (update.relinarizationNeeded(update_count_)) {
    ...
    if(debug_print_) std::cout << "inside the update.relinarizationNeeded block in ISAM2.cpp" << std::endl;
    ...
    if (!relinKeys.empty()) {
      ...
      if(debug_print_) std::cout << "Updating relinearization point " << std::endl;
  ...
  if(debug_print_) std::cout << "returning " << std::endl;
  return result;
}
...
Values ISAM2::calculateEstimateConstrained() {
  gttic(ISAM2_calculateEstimate);
  const VectorValues& delta(getDeltaConstrained());
  gttic(Expmap);
  return theta_.retract(delta);
  gttoc(Expmap);
}
...
/* ************************************************************************* */
const VectorValues& ISAM2::getDeltaConstrained() {
  double res;
  if (!deltaReplacedMask_.empty()) updateDeltaConstrained(&res, false);
  return delta_;
}

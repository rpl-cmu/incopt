...
inline static void optimizeInPlaceOtherRHS(const ISAM2::sharedClique& clique,
                                           VectorValues* result,
                                           NonlinearFactorGraph nonlinearFactors_,
                                           int iter,
                                           double* residualPrimal,
                                           const std::map<std::string, double>& constraintPenaltyParams_) {
  // parents are already be solved and available in result
  std::pair<VectorValues, Vector>  ret = clique->conditional()->solveConstrained(*result, clique->y_);
  result->update(ret.first);
  //result->capInPlace(1);
  map<int, tuple<FastVector<double>*, FastVector<double>*, FastVector<double>*>> cliqueMultiplierUpdate;
  for(auto &v: clique->factorIndices_) {
  ...
  clique->updateMultiplierTerms(cliqueMultiplierUpdate, const_cast<VectorValues*>(result), iter, residualPrimal, constraintPenaltyParams_);
  // starting from the root, call optimize on each conditional
  for (const ISAM2::sharedClique& child : clique->children)
    optimizeInPlaceOtherRHS(child, result, nonlinearFactors_, iter, residualPrimal, constraintPenaltyParams_);
}
...
// BEGIN CONSTRAINTS EDITS
/* ************************************************************************* */
size_t DeltaImpl::UpdateGaussNewtonDeltaOtherRHS(const ISAM2::Roots& roots,
  ...
  if (wildfireThreshold <= 0.0) {
    ...
    for (const ISAM2::sharedClique& root : roots)
      internal::optimizeInPlaceOtherRHS(root, delta, nonlinearFactors_, iter, residualPrimal, constraintPenaltyParams_);
    lastBacksubVariableCount = delta->size();
  } else {
    // Optimize with wildfire
    lastBacksubVariableCount = 0;
    for (const ISAM2::sharedClique& root : roots)
      lastBacksubVariableCount += optimizeWildfireNonRecursiveOtherRHS(
          root, wildfireThreshold, replacedKeys, delta, nonlinearFactors_, iter, residualPrimal, stopCliques, constraintPenaltyParams_);  // modifies delta

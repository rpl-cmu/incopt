...
namespace gtsam {
  ...
  EliminationTree<BAYESNET,GRAPH>::EliminationTree(const FactorGraphType& graph,
    ...
    boost::optional<FastVector<int>> factorIndices)
  {
    gttic(EliminationTree_Contructor);
    //std::cout << "Inside inference/EliminationTree-inst.h EliminationTree " << std::endl;
      ...
      for (size_t j = 0; j < n; j++)
      {
        ...
        for(const size_t i: factors) {
          ...
          if (prevCol[i] != none) {
            ...
            if(factorIndices) {
              node->factorIndices.push_back((*factorIndices)[i]);
            }
            else {
              node->factorIndices.push_back(i);
            }
            factorUsed[i] = true;
          }
          prevCol[i] = j;

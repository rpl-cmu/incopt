...
typedef FastSet<FactorIndex> FactorIndexSet;
  ...
  protected:
    ...
    // BEGIN CONSTRAINT EDITS
    // also added to constructors
    bool isConstraintFactor_;  
    int constraintFactorType_; // 0: none, 1: equality, 2: inequality greater than, 3: inequality lt
    FastVector<double> dualV_;
    FastVector<double> rho_;
    FastVector<double> constraintViolation_;
    // END CONSTRAINT EDITS

    /** Default constructor for I/O */
    Factor(): isConstraintFactor_(false), constraintFactorType_(0) {}

    /** Construct factor from container of keys.  This constructor is used internally from derived factor
    *  constructors, either from a container of keys or from a boost::assign::list_of. */
    template<typename CONTAINER>
    explicit Factor(const CONTAINER& keys) : keys_(keys.begin(), keys.end()), isConstraintFactor_(false), constraintFactorType_(0) {}
    ...
    *  factor constructors, although our code currently does not use this. */
    ...
    Factor(ITERATOR first, ITERATOR last) : keys_(first, last), isConstraintFactor_(false), constraintFactorType_(0) {}
  ...
  public:
    ...
    */
    ...
    // BEGIN CONSTRAINT EDITS
    bool isConstraintFactor() { return isConstraintFactor_; }
    int constraintFactorType() { return constraintFactorType_; }

    void setIsConstraintFactor(bool val) { isConstraintFactor_ = val; }
    void setConstraintFactorType(int val) { 
      if(val < 0 && val > 3) {
        val = 0;
      }
      constraintFactorType_ = val; 
    }

    void setdualV(FastVector<double> dualV) {
      dualV_ = dualV;
    }

    FastVector<double>* getdualV() {
      return &dualV_;
    }

    void setRho(FastVector<double> rho) {
      rho_ = rho;
    }

    FastVector<double>* getRho() {
      return &rho_;
    }

    void setConstraintViolation(FastVector<double> constraintViolation) {
      constraintViolation_ = constraintViolation;
    }

    FastVector<double>* getConstraintViolation() {

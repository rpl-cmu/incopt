...
struct ConstructorTraversalData {
  ...
  static ConstructorTraversalData ConstructorTraversalVisitorPre(
    ...
    // Note that myJTNode contains the original Gaussian factors (when using ISAM2)
    myData.myJTNode = boost::make_shared<Node>(node->key, node->factors);
    // Potentially update constructor to initialize? copy elimination tree node factorindices 
    // to cluster tree node  
    myData.myJTNode->factorIndices = node->factorIndices;

...
void ClusterTree<GRAPH>::Cluster::merge(const boost::shared_ptr<Cluster>& cluster) {
  ...
  factorIndices.insert(factorIndices.end(), cluster->factorIndices.begin(), cluster->factorIndices.end());
...
void ClusterTree<GRAPH>::Cluster::mergeChildren(
  ...
  factorIndices.reserve(nrFactors);
...
struct EliminationData {
  ...
  static EliminationData EliminationPreOrderVisitor(
    ...
    assert(node);
    //std::cout << "Inside EliminationPreOrderVisitor | node->nrChildren() = " << node->nrChildren() << std::endl;
  ...
  public:
    ...
    EliminationPostOrderVisitor(
        ...
        eliminationFunction_(eliminationFunction), nodesIndex_(nodesIndex), enableConstraints_(enableConstraints) {
    }
    ...
    void operator()(const typename CLUSTERTREE::sharedNode& node, EliminationData& myData) {
      ...
      if(enableConstraints_) {
        // gather the linearized constraint factors
        FastVector<sharedFactor> constraintFactors;
        FastVector<int> factorIndices;
        for(size_t i = 0; i< node->factors.size(); i++) {
            auto f = node->factors[i];
            if (f->isConstraintFactor()) {
              constraintFactors.push_back(f);
              factorIndices.push_back(node->factorIndices[i]);
            }
        }
        // Store linearized constraint factors in Bayes Tree clique node
        myData.bayesTreeNode->cacheConstraintFactors(constraintFactors, factorIndices);
      }
...
EliminatableClusterTree<BAYESTREE, GRAPH>::eliminate(const Eliminate& function) const {
  ...
  typename Data::EliminationPostOrderVisitor visitorPost(function, result->nodes_, this->enableConstraints_);
  {

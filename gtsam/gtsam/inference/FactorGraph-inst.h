...
FactorIndices FactorGraph<FACTOR>::add_factors(const CONTAINER& factors,
  ...
  if (useEmptySlots) {
    ...
    for (size_t j = 0; j < num_factors; ++j) {
      ...
      factorIndexToconstraintFactorType_[i] = factors[j]->constraintFactorType();
      if(factorIndexToconstraintFactorType_[i] == 1) {
          Neq_ += 1;
      }
      else if (factorIndexToconstraintFactorType_[i] == 2 || factorIndexToconstraintFactorType_[i] == 3 ) {
          Nieq_ += 1;
      }
    }
  } else {
    ...
    for (size_t i = 0; i < num_factors; ++i) {
      newFactorIndices[i] = i + size();
      factorIndexToconstraintFactorType_[newFactorIndices[i]] = factors[i]->constraintFactorType();
      if(factorIndexToconstraintFactorType_[newFactorIndices[i]] == 1) {
          Neq_ += 1;
      }
      else if (factorIndexToconstraintFactorType_[newFactorIndices[i]] == 2 || factorIndexToconstraintFactorType_[newFactorIndices[i]] == 3 ) {
          Nieq_ += 1;
      }
    }
    push_back(factors);

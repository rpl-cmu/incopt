...
class NonlinearFactorGraph {
  ...
  double unwhitenedError(const gtsam::Values& values) const;
...
virtual class NonlinearFactor {
  ...
  void printfromnonlinearfactor() const;
  bool isConstraintFactor();
  void setIsConstraintFactor(bool val);
  ...
  gtsam::Vector errorConstrained(const gtsam::Values& x) const;
  double error(const gtsam::Values& c) const;
  size_t dim() const;
  bool active(const gtsam::Values& c) const;
  gtsam::GaussianFactor* linearize(const gtsam::Values& c) const;
  gtsam::Vector errorConstrained(const gtsam::Values& c) const;
  double errorUnwhitened(const gtsam::Values& c) const;
...
class ISAM2Params {
  ...
  bool isForceConstrainedDeltaUpdate() const;
  void setForceConstrainedDeltaUpdate(bool forceConstrainedDeltaUpdate);
};
...
class ISAM2Result {
  ...
  double getNonLinearConstrainedError();
};
...
class ISAM2 {
 ...
 //void update(const gtsam::NonlinearFactorGraph& newFactors, const gtsam::Values& newTheta, const gtsam::KeyVector& removeFactorIndices, FastMap<Key,int>& constrainedKeys);
  ...
  void setEnableConstraints(bool flag);
  ...
  gtsam::Values calculateEstimateConstrained();
  gtsam::VectorValues& getDeltaConstrained();
...
virtual class BetweenFactor : gtsam::NoiseModelFactor {
  BetweenFactor(size_t key1, size_t key2, const T& d, const gtsam::noiseModel::Base* noiseModel);
  T measured() const;

...
def generateArm(arm_str, base_pose=None):
    ...
    if base_pose is None:
        base_pose = gtsam.Pose3(gtsam.Rot3(np.identity(3)), gtsam.Point3(np.asarray([0, 0, 0])))
    ...
    if arm_str is "SimpleTwoLinksArm":
        ...
        #arm = incopt.thirdparty.kinematics.Arm(2, a, alpha, d)
        arm = incopt.Arm(2, a, alpha, d)
        #% physical arm
        spheres_data = [
            [0, -0.5, 0.0, 0.0, 0.01],
            [0, -0.4, 0.0, 0.0, 0.01],
        ...
        for i in range(nr_body):
            sphere_vec.append(
                incopt.BodySphere(
                    int(spheres_data[i, 0]), spheres_data[i, 4], np.array(spheres_data[i, 1:4])
                )
            )
        arm_model = incopt.RobotModelArm(arm, np.array(sphere_vec))
    ...
    #% 3 link arm
        ...
        spheres_data = np.asarray(spheres_data)
        nr_body = spheres_data.shape[0]
        sphere_vec = incopt.BodySphereVector()
        for i in range(nr_body):
            sphere_vec.push_back(
                incopt.BodySphere(
                    spheres_data[i, 0], spheres_data[i, 4], gtsam.Point3(spheres_data[i, 1:4])
                )
            )
        arm_model = incopt.ArmModel(arm, sphere_vec)
    ...
    #% 7 link WAM arm
        ...
        abs_arm = incopt.Arm(7, a, alpha, d, base_pose, theta)
        ...
        arm_model = incopt.RobotModelArm(abs_arm, np.array(sphere_vec))
